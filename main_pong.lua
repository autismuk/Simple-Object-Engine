...
function Bat:onMessage(message)
	local ball = message.sender 																-- the ball sent the message
...
function Ball:onMessage(message)
	if message.contents[1] == "start" then  													-- if start is received
...
function Ball:onUpdate(dTime,dMilliTime)
	...
	if self.ball.x < 0 then  																	-- if off the left
		...
		if self:query("ball") == nil then SOE:deleteAll() end 									-- if there are no objects called 'ball' out there then game over 
	else
...
-- 	This last bit is nothing to do with Pong. It just shows a timer at work.
